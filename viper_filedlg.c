...
WINDOW*
viper_filedlg_create(WINDOW *parent, gchar *title, gfloat x, gfloat y,
...
gint
viper_kbd_default_FILEDLG_MENU(gint32 keystroke, WINDOW *window)
{
    VIPER_FILEDLG   *viper_filedlg;
    ...
    {
        case KEY_UP:
            menu_driver(viper_filedlg->file_menu, REQ_UP_ITEM);     break;
        case KEY_DOWN:
            menu_driver(viper_filedlg->file_menu, REQ_DOWN_ITEM);   break;
        case KEY_NPAGE:
            menu_driver(viper_filedlg->file_menu, REQ_SCR_DPAGE);   break;
        case KEY_PPAGE:
            menu_driver(viper_filedlg->file_menu,REQ_SCR_UPAGE);    break;
        case ' ':
            menu_driver(viper_filedlg->file_menu,REQ_TOGGLE_ITEM);  break;
    }
    ...
    if(keystroke == KEY_CRLF)
    {
        ...
        {
            file_list = filedlg_readdir(viper_fstats->filepath,
                viper_filedlg->flags);
            ...
            g_slist_foreach(viper_filedlg->fstats_list,
                g_slist_free_data, NULL);
...
gint
viper_kbd_default_FILEDLG_PATH(gint32 keystroke, WINDOW *window)
{
    VIPER_FILEDLG   *viper_filedlg;
...
gint
viper_filedlg_ON_MOVE(WINDOW *window, gpointer arg)
{
    VIPER_FILEDLG   *viper_filedlg;
...
gint
viper_filedlg_ON_CLOSE(WINDOW *window, gpointer arg)
{
    VIPER_FILEDLG   *viper_filedlg;
    VIPER_FSTATS    *viper_fstats;
    GSList          *fstats_list;
        ...
        else
        {
            ...
            while(*items != NULL)
            {
                if(item_value(*items))
                {
                    ...
                    viper_fstats = (VIPER_FSTATS*)g_slist_nth_data(fstats_list,
                        idx);
                    results[sz_buf - 1] = g_strndup(viper_fstats->filepath,
                        NAME_MAX);
                }
                items++;
            }
...
GSList*
filedlg_readdir(gchar *dir, gint32 flags)
{
    DIR             *directory;
    struct dirent   *entry;
...
gchar**
filedlg_format_items(GSList *fstats_list, gint32 flags, gint width)
{
    VIPER_FSTATS    *viper_fstats;
    gchar           **items;
    GSList          *node;
    ...
    while(node != NULL)
    {
        ...
        if(flags & FILEDLG_SHOW_SIZE)
        {
            ...
            sprintf(&items[idx][offset - 1], "|%*s",
                FIELD_WIDTH_SIZE - 1, buffer);
...
void
filedlg_field_size(off_t size,gchar *buffer)
{
...
void
filedlg_field_ctime(time_t ctime, gchar *buffer)
{
...
void
filedlg_field_permissions(mode_t mode, gchar *buffer)
{
    memset(buffer, '-', 10);
    buffer[10] = 0;
...
void
g_slist_free_data(gpointer data, gpointer anything)
{
    g_free(data);
    return;
}
...
gint
g_slist_sort_func(gconstpointer a, gconstpointer b)
{
    gchar   *string1;
    gchar   *string2;
